...
from .forms import UserBookForm, ProfileForm, UserEditForm, RatingForm, CreateUserForm, LoginForm
...
def edit_profile(request):
    if request.method == 'POST':
        ...
        profile_form = ProfileForm(request.POST, request.FILES, instance=request.user.edit_profile)
        ...
        try:
            profile_form = ProfileForm(instance=request.user.edit_profile)
...
def registr(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return render(request, 'blog/registration/reg.html', {'form': form})
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                form.add_error(request, 'Неверные учетные данные')
    else:
        form = LoginForm()    
...
def error(request):
    return render(request, 'blog/Error/error.html')